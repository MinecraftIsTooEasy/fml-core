...
public class ModListRequestPacket extends FMLPacket
{
    ...
    public FMLPacket consumePacket(byte[] data)
    {
        ...
        {
            FMLLog.fine("No compatibility byte found - the huix.mixins.server is too old");
        }
        return this;
    }
    ...
    /**
     *
     * This packet is executed on the client to evaluate the huix.mixins.server's mod list against
     * the client
     *
            ...
            for (Entry<String, ModContainer> e : indexedModList.entrySet())
            {
                if (e.getValue().isNetworkMod())
                {
                    ...
                    if (missingHandler.requiresServerSide())
                    {
                        ...
                        FMLLog.warning("The mod %s was not found on the huix.mixins.server you connected to, but requested that the huix.mixins.server side be present", e.getKey());
                    }
                }
            }
        }
