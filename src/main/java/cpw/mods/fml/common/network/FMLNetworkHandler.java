...
public class FMLNetworkHandler
{
    ...
    // List of states for connections from clients to huix.mixins.server
    ...
    public static void handlePacket250Packet(Packet250CustomPayload packet, INetworkManager network, NetHandler handler)
    {
        ...
        {
            ((IINetClientHandler) handler).handleVanilla250Packet(packet);
        }
    ...
    private void handleFMLPacket(Packet250CustomPayload packet, INetworkManager network, NetHandler netHandler)
    {
        ...
        {
            userName = NetLoginHandler.getClientUsername((NetLoginHandler) netHandler);
        }
        else
        {
            EntityPlayer pl = ((IINetClientHandler) netHandler).getPlayer();
            if (pl != null)
            {
                userName = pl.getEntityName();
            }
        }
    ...
    private void handleClientConnection(NetLoginHandler netLoginHandler, MinecraftServer server, SocketAddress address, String userName)
    {
        ...
        case LOGIN_RECEIVED:
            ...
            // Send the mod list request packet to the client from the huix.mixins.server
    ...
    private boolean handleVanillaLoginKick(NetLoginHandler netLoginHandler, MinecraftServer server, SocketAddress address, String userName)
    {
        ...
        BanEntry banEntry;
        if (playerList.getBannedPlayers().getBannedList().get(userName) != null) {
            banEntry = (BanEntry) playerList.getBannedPlayers().getBannedList().get(userName);
        } else {
            banEntry = (BanEntry) playerList.getBannedIPs().getBannedList().get(address);
        }


        String kickReason = banEntry.getBanReason();
    ...
    public static void openGui(EntityPlayer player, Object mod, int modGuiId, World world, int x, int y, int z)
    {
        ...
        if (mc == null)
        {
            ...
            {
                FMLLog.warning("A mod tried to open a gui on the huix.mixins.server without being a NetworkMod");
                return;
            }
        }
        ...
        {
            FMLLog.fine("Invalid attempt to open a local GUI on a dedicated huix.mixins.server. This is likely a bug. GUIID: %s,%d", mc.getModId(), modGuiId);
        }
    }
